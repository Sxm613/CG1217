                           ...
                           bool generate_mipmap)
{
	...
	//! \todo repeat now the texture filling for the 5 remaining faces
		auto loadFace = [&](const std::string& path, GLenum face) {
	    auto data = getTextureData(path, width, height, false);
	    if (data.empty()) {
	        glDeleteTextures(1, &texture);
	        return false;
	    }
	    glTexImage2D(face, 0, GL_RGBA, static_cast<GLsizei>(width), static_cast<GLsizei>(height), 0,
	                 GL_RGBA, GL_UNSIGNED_BYTE, reinterpret_cast<GLvoid const*>(data.data()));
	    return true;
	};

	 // Fill the cube map with the texture data
	if (!loadFace(posx, GL_TEXTURE_CUBE_MAP_POSITIVE_X) ||
		!loadFace(negx, GL_TEXTURE_CUBE_MAP_NEGATIVE_X) ||
		!loadFace(posy, GL_TEXTURE_CUBE_MAP_POSITIVE_Y) ||
		!loadFace(negy, GL_TEXTURE_CUBE_MAP_NEGATIVE_Y) ||
		!loadFace(posz, GL_TEXTURE_CUBE_MAP_POSITIVE_Z) ||
		!loadFace(negz, GL_TEXTURE_CUBE_MAP_NEGATIVE_Z)) {
		return 0u; // Handle the error
	}
