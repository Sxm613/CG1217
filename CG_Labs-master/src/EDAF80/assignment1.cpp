...
//void renderScene(CelestialBody& root, glm::mat4 const& view_projection, bool show_basis, std::chrono::microseconds elapsed_time)
void renderScene(CelestialBody& root, glm::mat4 const& view_projection, bool show_basis, std::chrono::microseconds elapsed_time, CelestialBody* target_body, glm::mat4& target_world_matrix, CelestialBody* root_body, glm::mat4& root_world_matrix)

{
	std::stack<CelestialBodyRef> stack;

	// 初始化栈
	CelestialBodyRef root_ref = { &root, glm::translate(glm::mat4(1.0f), glm::vec3(2.0f, 0.0f, 0.0f)) };
	stack.push(root_ref);

	while (!stack.empty())
	{
		CelestialBodyRef current_ref = stack.top();
		stack.pop();

		// 渲染当前天体
		//glm::mat4 world = current_ref.body->render(elapsed_time, view_projection, current_ref.parent_transform, show_basis);
		// 假设 render 函数仍然返回 std::pair<glm::mat4, glm::mat4>
		std::pair<glm::mat4, glm::mat4> result = current_ref.body->render(elapsed_time, view_projection, current_ref.parent_transform, show_basis);

		// 获取 world 矩阵
		glm::mat4 world = result.first; // 获取 std::pair 中的第一个元素，即 world 矩阵

		glm::mat4 parent = result.second; 

		// 如果是目标天体，保存它的世界矩阵
		if (current_ref.body == target_body)
		{
			target_world_matrix = parent;
		}

		if (current_ref.body == root_body)
		{
			root_world_matrix = parent;
		}

		// 将子节点推入栈中
		for (auto* child : current_ref.body->get_children())
		{
			glm::mat4 child_parent_transform = parent; // 当前天体的世界矩阵作为子天体的父变换矩阵
			stack.push(CelestialBodyRef{ child, child_parent_transform });
		}
	}
}

glm::vec3 getPositionFromMatrix(glm::mat4 const& matrix)
{
	//glm::vec3 position = glm::vec3(matrix[3]); // 从矩阵的第四列提取平移部分
	glm::vec3 position = glm::vec3(matrix[3].x, matrix[3].y, matrix[3].z);
	return position;
...
int main()
{
	...
	//earth.set_scale(glm::vec3(0.1f, 0.1f, 0.1f));
	...
	sun.set_spin(sun_spin); // No orbit, just spin
	...
	glm::mat4 root_world_matrix;
	CelestialBody* target_planet = &earth; 
	CelestialBody* root_planet = &sun;
	...
	while (!glfwWindowShouldClose(window)) {
		...
		//glm::mat4 world_orbit_matrix = earth.render(animation_delta_time_us, camera.GetWorldToClipMatrix(), glm::translate(glm::mat4(1.0f), glm::vec3(2.0f, 0.0f, 0.0f)), show_basis);

		//moon.render(animation_delta_time_us, camera.GetWorldToClipMatrix(), world_orbit_matrix, show_basis);
		
		// 在适当的位置调用 renderScene，例如在主循环中
		//renderScene(sun, camera.GetWorldToClipMatrix(), show_basis, animation_delta_time_us);
		renderScene(sun, camera.GetWorldToClipMatrix(), show_basis, animation_delta_time_us, target_planet, target_world_matrix, root_planet, root_world_matrix);


		// 获取目标星球的位置
		glm::vec3 target_position = getPositionFromMatrix(target_world_matrix);
		glm::vec3 root_position = getPositionFromMatrix(root_world_matrix);



		glm::vec3 direction_to_target = glm::normalize(target_position - root_position);
		// 设置相机，使其跟随目标星球
		
		glm::vec3 camera_position = target_position + direction_to_target * 2.0f;

		camera.mWorld.SetTranslate(camera_position);

		camera.mWorld.LookAt(direction_to_target);
